...
#         raise HTTPException(
#             status_code=400,
#             detail="Only PDF, TXT, DOCX, RTF files are allowed"
#         )

#     # STEP 2: FIXED folder handling (PUT YOUR CODE HERE)
#     dir_path = "DATA/raw"
...
#     # STEP 3: Save file
#     file_path = os.path.join(dir_path, f"{int(time.time())}_{file.filename}")
...
#     # STEP 4: Process file
#     pipeline.ingest(file_path, user_id)
...
#     return {"status": f"{file.filename} processed successfully"}


# @router.get("/ask")
# def ask_question(question: str, user_id: int = 1):
#     answer = pipeline.answer(question, user_id)
...
#         (user_id, question, formatted_answer)
#     )
...
# def summarize(user_id: int = 1, top_k: int = 10):
#     summary = pipeline.summarize(user_id=user_id, top_k=top_k)
#     return {"summary": summary}



# @router.get("/generate-questions")
# def generate_questions(user_id: int = 1, num_questions: int = 10):
#     questions = pipeline.generate_questions(user_id=user_id, num_questions=num_questions)
#     return {"questions": questions}


# @router.get("/generate-quiz")
# def generate_quiz(user_id: int = 1, num_questions: int = 5):
#     quiz = pipeline.generate_quiz(user_id=user_id, num_questions=num_questions)
#     return {"quiz": quiz}



# @router.post("/mock-test/start")
# def start_mock_test(user_id: str, topic: str):
    
#     # Basic validation
#     if not user_id or not topic:
#         return {"error": "user_id and topic are required"}

#     service = MockTestService(llm)
#     tracker = TrackingService()

#     #Adaptive difficulty
#     difficulty = service.get_adaptive_difficulty(user_id, tracker)

#     #Generate questions
#     questions = service.generate_mcqs(topic, difficulty=difficulty)
...
#     # Basic validation
#     if not user_id or not questions or not user_answers:
#         return {"error": "Missing required fields"}

#     service = MockTestService(llm)

#     # Evaluate
#     score, results = service.evaluate_mcq(questions, user_answers)
...
#     # STEP 1: File type validation (PUT IT HERE)
#     allowed_extensions = [".pdf", ".txt", ".docx", ".rtf"]
...
# def mock_test(user_id: str, topic: str, difficulty: str = None):
#     service = MockTestService(llm)
#     tracker = TrackingService()
...
#     # Generate questions based on student level
#     questions = service.generate_mcqs(topic, difficulty=difficulty)
...
#     return {"status": "PDF processed + indexed"}
...
#     os.makedirs("DATA/raw", exist_ok=True)   #  CREATE FOLDER FIRST
...
#     #  IMPORTANT CHANGE: pass file_path, NOT chunks
#     pipeline.ingest(file_path)
